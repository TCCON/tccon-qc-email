...
class BibtexFormMixin:
    ...
    @classmethod
    def load_citation_instance(cls, citation_type, site_id):
        site_info = cls.load_citation_dict(citation_type, site_id)
        if len(site_info) == 0:
            return None

        if 'bibtex_type' not in site_info:
            # Should not happen, there should always
            raise TypeError('Missing expected bibtex_type')

        for klass in cls.__subclasses__():
            if klass.bibtex_type == site_info['bibtex_type']:
                return klass(site_info)

        raise TypeError(f'No matching form for type {site_info["bibtex_type"]} found')
    ...
    @staticmethod
    def _gen_string():
        return f'% Generated by tccondata.org on {timezone.now():%Y-%m-%d %H:%M:%S %Z}'

    @classmethod
    def generate_bibtex(cls, site_ids, bibtex_key_prefix='tccon-', include_site_refs=False):
        bibtex = StringIO('')
        bibtex.write(f'{cls._gen_string()}\n\n')
        citation_types = ('siteref', 'dataref') if include_site_refs else ('dataref',)
        for site in site_ids:
            for citation in citation_types:
                try:
                    form = cls.load_citation_instance(citation, site)
                except TypeError:
                    bibtex.write(f'% Sorry, could not generate a {citation} for TCCON site {site}')
                else:
                    if form is None:
                        bibtex.write(f'% No {citation} available for {site}')
                    else:
                        form.bibtex_citation(citation, site, bibtex_key_prefix, stream=bibtex)
    ...
    def generate_latex_table(cls, site_ids, bibtex_key_prefix='tccon-', citation_cmd='cite', include_site_refs=False):
        ...
        for site_id in site_ids:
            ...
            if include_site_refs:
                site_bib_key = cls._make_bibtex_key(site_id=site_id, citation_type='siteref', key_prefix=bibtex_key_prefix)
                if site_bib_key:
                    site_citation = f'\\{citation_cmd}{{{site_bib_key}}}'
                else:
                    site_citation = '-'
                table_rows.append(f'{site_id} & {site_name} & {location} & {data_citation} & {site_citation}')
            else:
    ...
    @classmethod
    def _make_bibtex_key(cls, site_id, citation_type, key_prefix):
        site_file = cls.get_site_file(site_id)
        if not site_file.exists():
            return None

        citations = InfoFileLocks.read_json_file(site_file)
        if citation_type in citations:
            return f'{key_prefix}{site_id}-{citation_type}'
        return None

    def bibtex_citation(self: Form, citation_type: str, site_id: str, key_prefix='tccon-', stream: StringIO = None):
        if stream is None:
            stream = StringIO('')
            return_string = True
        else:
        ...
        key = self._make_bibtex_key(site_id=site_id, citation_type=citation_type, key_prefix=key_prefix)
        stream.write(f'@{self.bibtex_type}{{{key},\n')
        fields_to_omit = self.omit_fields()
