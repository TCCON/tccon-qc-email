...
def send_email(subject, body, send_from, send_to, attachment, smtp_args=None, authenticate=False, dry_run=False):
    """
    ...
    if not dry_run:
        ...
        print('[dry run] Start SMTP with arguments:', smtp_args)
        if authenticate:
            print(f'[dry run] Authenticate with email = {send_from}, password = *****')
        print(f'[dry run] Send mail from {send_from} to {send_to}: {message.as_string()}')


def send_email_ext(subject, body, send_from, send_to, attachment, cfg, dry_run=False):
...
def send_email_from_config(cfg_file, site_id, attachment, nc_file, plot_url=None, dry_run=False):
    ...
    # Also if there is a command to do so, get the editor and reviewers for this site
    reviewer_cmd = cfg['email'].get('reviewer_cmd', None)
    reviewers = None
    if reviewer_cmd and reviewer_cmd == '__internal__':
        response = requests.get(f'https://tccondata.org/metadata/qcform/api/v1/getreviewers/{site_id}')
        if response.status_code == 200:
            reviewers = json.loads(response.content)
    elif reviewer_cmd:
        reviewer_cmd = shlex.split(reviewer_cmd)
        reviewer_cmd.append(site_id)
        # If this fails, 
        result = run(reviewer_cmd, capture_output=True)
    ...
    if cfg['server']['use_external_program']:
        ...
        send_email(subject=subject, body=body, send_from=from_addr, send_to=to_addr, attachment=attachment,
                   smtp_args=smtp_args, authenticate=auth, dry_run=dry_run)
